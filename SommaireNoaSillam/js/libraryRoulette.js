...
$.fn.playSpin = function (options) {
    if (this.length) {
        if ($(this).is(':animated')) return; // renvoie false si cet element est en cours d'animation
...
$.fn.stopSpin = function () {
    if (this.length) {
        if (!$(this).is(':animated')) return; // Renvoie false si cet élément n'est pas en cours d'animation
...
var slotMachine = function (el, options, track) {
    ...
    slot.defaultOptions = {
        easing: 'swing',        // String: chaine de caracterer
        time: 3000,             // Number: durée total de l'animation de rotation
        loops: 6,               // Number: nombre de fois qu'il tourne pendant l'animation
        manualStop: false,      // Boolean: tourner jusqu'a que l'utilisateur click pour l'arreter
        stopSeq: 'random',      // String: séquence d'animation machine à sous aleatoire à gauche
        endNum: 0,              // Number: fin de l'animation à quel numéro / séquence de la liste
        onEnd : $.noop,         // Function: exécuter à chaque extrémité de rotation d'élément, il est passé endNum
        onFinish: $.noop        // Function: s'exécute sur toute la fin de rotation de l'élément, il est passé endNum
    };
    ...
    slot.setup = function () {
        ...
        $li.clone().appendTo(slot.$el); // Cloner à la dernière ligne pour une animation fluide
    ...
    slot.endSpin = function () {
        ...
        // Gestion des erreurs si endNum est hors de portée
        ...
        slot.$el
            ...
            .animate({'top': finalPos}, finalSpeed, slot.options.easing, function () {
                slot.$el.find('li').last().remove(); // Supprimer la ligne clonée
                ...
                // onFinish est l'animation de chaque élément est terminée
